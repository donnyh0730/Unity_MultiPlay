...
namespace Server.GameContents
{
    ...
    public class Map
    {
        public int MinX { get; set; }// -23
        public int MaxX { get; set; }// 23
        public int MinY { get; set; }// -10
        public int MaxY { get; set; }// 14
        ...
        public bool CanGo(Vector2Int cellPos, bool cheackObjects = true)
        {
            ...
            return !_collision[y, x] && (!cheackObjects || _players[y,x] == null);
        }

        public Player Find(Vector2Int cellPos)
        {
            if (!IsInMapPos(cellPos))
                return null;

            Pos pos = Cell2Pos(cellPos);

            return _players[pos.Y, pos.X];
        }

        public bool ApplyMove(Player player, Vector2Int dest)
        {
            PositionInfo playerposInfo = player.Info.PosInfo;

            if (IsInMapPos(new Vector2Int(playerposInfo.PosX, playerposInfo.PosY)) == false)
                return false;
            ...
            {
                int x = playerposInfo.PosX - MinX;
                int y = MaxY - playerposInfo.PosY;
            ...
            playerposInfo.PosX = dest.x;
            playerposInfo.PosY = dest.y;
        ...
        public bool IsInMapPos(Vector2Int pos)
        {
